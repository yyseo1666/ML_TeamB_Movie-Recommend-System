"""
The Movies Dataset(ratings.csv)를 이용한 SVD 추천

과정:
1) ratings 데이터를 이용해 사용자–아이템 평점 행렬 생성 -> 초기 평점 행렬 R_nan 출력/저장
...
디렉터리 구조:
- 이 파일(.py)과 the-movies/ 폴더가 같은 디렉터리에 존재
- the-movies/ 아래에 ratings.csv 존재
- 저장된 결과 파일(.csv, .npy)은 이 파일과 같은 디렉터리에 생성되는 SVD-The_Movies_outputs 폴더 안에 존재
"""
...
# 설정값들
# =========
DATA_DIRNAME = "the-movies"                    # 데이터 폴더명
...
def load_kaggle_ratings(
    ...
    """
    Kaggle The Movies Dataset의 ratings.csv를 불러와서
    1) user_id / item_id 컬럼으로 rename
    2) 인기 많은 사용자/영화 위주로 잘라서 (메모리 폭발 방지)
    3) train/test로 랜덤 분할
    """
    # 0) 읽기
    ...
    # 기존 코드와 동일하게 컬럼은 user_id, item_id, rating, timestamp 를 유지
    return train, test
